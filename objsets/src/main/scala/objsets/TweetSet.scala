...
class Tweet(val user: String, val text: String, val retweets: Int) {
  ...
  def maxRetweeted(other: Tweet): Tweet = if(retweets>other.retweets) this else other
  
  def speaksOf(word: String): Boolean = text.contains(word)
}
...
class Empty extends TweetSet {
  ...
  def mostRetweeted = throw new NoSuchElementException("Empty TweetSets have no mostRetweeted")
...
class NonEmpty(val elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted = 
    if(this.isLeaf)
      ...
      elem.maxRetweeted(this.remove(elem).mostRetweeted)    
...
object GoogleVsApple {
  ...
  def tweetsMentioning(keywords: List[String]): TweetSet =
    TweetReader.allTweets.filter{tweet => keywords.exists{word => tweet.speaksOf(word)}} 
